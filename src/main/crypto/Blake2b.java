...
import static main.crypto.Blake2b.Engine.Assert.exclusiveLowerBound;
import static main.crypto.Blake2b.Engine.Assert.inclusiveLowerBound;
import static main.crypto.Blake2b.Engine.Assert.inclusiveUpperBound;
import static main.crypto.Blake2b.Engine.LittleEndian.readLong;
import static main.crypto.Blake2b.Engine.LittleEndian.writeLong;
...
import main.crypto.Blake2b.Engine.LittleEndian;
...
public interface Blake2b {
    ...
    interface Spec {
        ...
        int MAX_PERSONALIZATION = 16;

        /** length of h space vector array */
        int STATE_SPACE_LEN = 8;

        /** max tree fanout value */
        int MAX_TREE_FANOUT = 0xFF;

        /** max tree depth value */
        int MAX_TREE_DEPTH = 0xFF;

        /**
         * max tree leaf length value.Note that this has uint32 semantics and thus 0xFFFFFFFF is
         * used as max value limit.
         */
        int MAX_TREE_LEAF_LEN = 0xFFFFFFFF;

        /**
         * max node offset value. Note that this has uint64 semantics and thus 0xFFFFFFFFFFFFFFFFL
         * is used as max value limit.
         */
        long MAX_NODE_OFFSET = 0xFFFFFFFFFFFFFFFFL;

        /** max tree inner length value */
        int MAX_TREE_INNER_LEN = 0xFF;
    ...
    class Mac extends Engine implements Blake2b {
        ...
        /**
         * Blake2b.MAC - using default Blake2b.Spec settings with given key, with given digest
         * length
         */
        ...
        /**
         * Blake2b.MAC - using default Blake2b.Spec settings with given java.security.Key, with
         * given digest length
         */
    ...
    // ---------------------------------------------------------------------
    /**
     * Note that Tree is just a convenience class; incremental hash (tree) can be done directly with
     * the Digest class. <br>
    ...
    class Engine implements Blake2b {
        ...
        // ---------------------------------------------------------------------
        /**
         * write out the digest output from the 'h' registers. truncate full output if necessary.
         */
        ...
        public static class Debug {
            ...
            public static void dumpBuffer(
                final PrintStream out, final String label, final byte[] b) {
            ...
            public static void dumpBuffer(
                final PrintStream out, final byte[] b, final int offset, final int len) {
        ...
        public static final class Assert {
            ...
            public static <T extends Number> String assertFail(
                final String name, final T v, final String err, final T spec) {
    ...
    class Param implements AlgorithmParameterSpec {
        ...
        public interface Default {
            ...
            byte digest_length = Spec.MAX_DIGEST;
            byte key_length = 0;
            byte fanout = 1;
            byte depth = 1;
            int leaf_length = 0;
        ...
        public Param clone() {
            ...
            if (this.hasKey) {
                ...
                clone.key_bytes = new byte[Spec.MAX_KEY * 2];
        ...
        public final Param setDigestLength(int len) {
            assert len > 0 : Engine.Assert.assertFail("len", len, exclusiveLowerBound, 0);
            assert len <= Spec.MAX_DIGEST
                : Engine.Assert.assertFail("len", len, inclusiveUpperBound, Spec.MAX_DIGEST);
        ...
        public final Param setKey(final byte[] key) {
            assert key != null : "key is null";
            assert key.length >= 0 : Engine.Assert.assertFail("key.length", key.length, inclusiveUpperBound, 0);
            assert key.length <= Spec.MAX_KEY
                : Engine.Assert.assertFail("key.length", key.length, inclusiveUpperBound, Spec.MAX_KEY);

            // zeropad keybytes
            this.key_bytes = new byte[Spec.MAX_KEY * 2];
        ...
        public final Param setFanout(int fanout) {
            assert fanout > 0 : Engine.Assert.assertFail("fanout", fanout, exclusiveLowerBound, 0);
        ...
        public final Param setDepth(int depth) {
            assert depth > 0 : Engine.Assert.assertFail("depth", depth, exclusiveLowerBound, 0);
        ...
        public final Param setLeafLength(int leaf_length) {
            assert leaf_length >= 0
                : Engine.Assert.assertFail("leaf_length", leaf_length, inclusiveLowerBound, 0);
        ...
        public final Param setNodeOffset(long node_offset) {
            assert node_offset >= 0
                : Engine.Assert.assertFail("node_offset", node_offset, inclusiveLowerBound, 0);
        ...
        public final Param setNodeDepth(int node_depth) {
            assert node_depth >= 0 : Engine.Assert.assertFail("node_depth", node_depth, inclusiveLowerBound, 0);
        ...
        public final Param setInnerLength(int inner_length) {
            assert inner_length >= 0
                : Engine.Assert.assertFail("inner_length", inner_length, inclusiveLowerBound, 0);
        ...
        public final Param setSalt(final byte[] salt) {
            ...
            assert salt.length <= Spec.MAX_SALT
                : Engine.Assert.assertFail("salt.length", salt.length, inclusiveUpperBound, Spec.MAX_SALT);

            lazyInitBytes();
            Arrays.fill(bytes, Xoff.salt, Xoff.salt + Spec.MAX_SALT, (byte) 0);
        ...
        public final Param setPersonal(byte[] personal) {
            ...
            assert personal.length <= Spec.MAX_PERSONALIZATION
                : Engine.Assert.assertFail(
                "personal.length",
                personal.length,
                inclusiveUpperBound,
                Spec.MAX_PERSONALIZATION);

            lazyInitBytes();
            Arrays.fill(bytes, Xoff.personal, Xoff.personal + Spec.MAX_PERSONALIZATION, (byte) 0);
